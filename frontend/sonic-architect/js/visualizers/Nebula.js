 ...
 * Nebula - Volumetric Space Nebula
 */
...
const fragmentShader = `
    ...
    void main() {
        vec3 p = vPosition * 0.5 + vec3(uTime * 0.05);

        float nebula = fbm(p);
        nebula += fbm(p + vec3(uTime * 0.1, 0.0, 0.0)) * 0.5;

        float density = smoothstep(-0.3, 0.8, nebula);
        density *= (1.0 + uBeatIntensity * 0.5);

        vec3 color1 = vec3(1.0, 0.2, 0.5);  // Pink
        vec3 color2 = vec3(0.2, 0.5, 1.0);  // Blue
        vec3 color3 = vec3(0.8, 0.2, 1.0);  // Purple

        vec3 color = mix(color1, color2, smoothstep(-0.5, 0.0, nebula));
        color = mix(color, color3, smoothstep(0.0, 0.5, nebula));

        color *= density;
        color += vec3(uBass * 0.3, uMid * 0.2, uHigh * 0.4);
...
class Nebula extends VisualizerBase {
    constructor(scene, camera, renderer) {
        super(scene, camera, renderer);

        this.id = 'nebula';
        this.name = 'Nebula';
        this.description = 'Volumetric space nebula';
